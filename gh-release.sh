...
GITHUB_RELEASES_CHECK=$(curl -s https://api.github.com/repos/${GITHUB_USER}/${GITHUB_REPOSITORY}/releases|grep '\"name\"'|grep 'pqChecker v2.0.0'|awk -F\" '{ print $(NF-1) }'|awk -F/ '{ print $NF }'|wc -l)
